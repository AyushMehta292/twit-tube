...
function TweetLike({ isLikedInitially, tweetId, totalLikes }) {
  const dispatch = useDispatch();
  const [like, setLike] = useState({ isLiked: isLikedInitially, totalLikes });

  const handleToggleLike = () => {
    dispatch(toggleTweetLike(tweetId)).then((res) => setLike(res.payload));
  };
  ...
  return (
    <div className="flex gap-4">
      <button
        ...
        // focus:after:content-[attr(data-like-count-alt)]
        data-like-count={like?.totalLikes}
        data-like-count-alt={like?.isLiked ? like?.totalLikes - 1 : like?.totalLikes + 1}
      >
        <svg
